...
ExcelOut = FDir & "\" & FName & "--compiled.xlsb"
...
Print ">> Disable Macro guards in the registry" & vbCrLf
...
For Each oFile in getFiles(MacroDir).Items
    ...
    If ext = ".cls" or ext = ".bas" or ext = ".frm" or ext = ".txt" Then
        I = I + 1
        If I < 4 Then
            Print ">> Inject: " & oFile.Path & vbCrLf
        ElseIf I = 4 Then
            Print ">> Inject: etc..." & vbCrLf
            Print vbCrLf
        End If
        ...
        If ext = ".txt" Then
            ...
            End With 
            if StrComp(CodeName, sheetname, 1) <> 0 then
                MsgBox "Failed to inject VBA module for sheet " & sheetname & "; no such sheet!"
            end If
        Else
